...
- **Stakes & money payouts** — pick a common base cost (10¢, 20¢, 25¢, 30¢, 50¢, $1, $2 at 1 tai) or type a custom amount. The **30¢ ("3/6") stake** is a hybrid: everyone-pays mode doubles from the 30¢ base like the other presets, but shooter mode uses a fixed $4/$7/$11/$20/$40 schedule from the discarder alone, and self-draw collects a fixed $2/$3/$5/$10/$20 from every player. A reference table shows what the **shooter** (discarder, pays double) and **non-shooters** pay at every tai level, plus **bite** payouts (open kong/animal = 1 tai, hidden kong = 2 tai, collected instantly from every player). Winning hands show the dollar amounts alongside the tai breakdown.
...
- **Tab integration** — a winning hand in the analyzer has a "Record this win in the game tracker" button (prefills tai and self-draw/discard; you pick the player); recording or undoing hands in the tracker keeps the analyzer's prevailing wind in sync; the tracker can import the analyzer's stakes with one tap.
